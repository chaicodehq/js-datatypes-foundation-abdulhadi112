...
export function repeatPattern(pattern, times) {
  ...
  if (isString(pattern) && isNumber(times) && times > 0 && Number.isInteger(times))
    return pattern.repeat(times)
  else
    return ""
...
export function extractRangoliCenter(design, start, end) {
  // Your code here
  // if (!isString(design) || !isNumber(start) || !isNumber(end))
  if (
...
export function replaceRangoliColor(design, oldColor, newColor) {
  // Your code here
  if (!isString(design) || !isString(oldColor) || !isString(newColor)) return ""
...
export function makeRangoliBorder(char, length) {
  // Your code here
  if (!isString(char) || length < 0 || typeof length !== 'number') return ""
